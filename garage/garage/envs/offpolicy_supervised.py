...
if __name__ == '__main__':
	...
	sess = tf.Session()
	src = "/home/tbla2725/garage/data/local/experiment/networks/untrained_ddpg_reacher10.pkl"
	dest = "/home/tbla2725/garage/data/local/experiment/networks/trained_ddpg_reacher10.pkl"
	with sess.as_default(): data = joblib.load(src)
	policy = data['policy']
	print("prepare data")
	obs, act = load_data('../data/reacher/trajectories.npz')
	X = obs
	y = act
	y = np.clip(y, -1.0, 1.0)
	...
	policy_output = policy._l_prob
	pred = policy.predict(input_var)
	pred_error = tf.reduce_mean(tf.square(pred - target_var), axis=0)
	l2_regularization = L.regularize_network_params(policy_output, tf.nn.l2_loss,
		{'regularizable': True})
