...
from garage.tf.core.network import MLP, ParamNet
...
class GaussianMLPPolicy(StochasticPolicy, LayersPowered, Serializable):
        ...
        with tf.variable_scope(name, "GaussianMLPPolicy"):
            ...
            if std_network is None:
                ...
                if adaptive_std:
                    ...
                    with tf.variable_scope(self._std_network_name):
                        std_network = ParamNet(
                            output_dim=action_dim,
                            input_layer=mean_network.input_layer,
                            initializer=tf.constant_initializer(init_std_param),
                        )
            std_network.input_layer.input_var = mean_network.input_layer.input_var
            ...
            self._outputs = dict(mean=self._l_mean, log_std=self._l_std_param)
    ...
    def log_diagnostics(self, paths):
        log_stds = np.ma.masked_equal(paths["agent_infos"]['log_std'], 0)
        logger.record_tabular("{}/AveragePolicyStd".format(self.name),
                              list(np.mean(np.exp(log_stds), axis=(0,1))))
