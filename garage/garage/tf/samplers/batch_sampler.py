...
class BatchSampler(BaseSampler):
    ...
    def process_samples(self, itr, paths):
        ...
        lower_bound = self.algo.env.action_space.low
        upper_bound = self.algo.env.action_space.high
        prob_in_bounds = self.algo.policy.distribution.prob_in_bounds(
            agent_infos, lower_bound, upper_bound)
        ...
        logger.record_tabular('ProbInBounds', np.mean(prob_in_bounds))
        logger.record_tabular('Perplexity', np.exp(ent))
        logger.record_tabular('StdReturn', np.std(undiscounted_returns))
        logger.record_tabular('MaxReturn', np.max(undiscounted_returns))
        logger.record_tabular('MinReturn', np.min(undiscounted_returns))
        logger.record_tabular('ActionsMean', np.mean(actions))
        logger.record_tabular('ActionsStd', np.std(actions))
