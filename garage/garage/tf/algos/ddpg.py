...
class DDPG(OffPolicyRLAlgorithm):
    ...
    def init_opt(self):
        with tf.name_scope(self.name, "DDPG"):
            ...
            if hasattr(self.policy, 'copy_net'):
                #TODO: initialize the policy network here
                #self.policy.initialize_net()
                target_policy = self.policy.copy_net(
                    trainable=False, name="target_policy")
                target_action = layers.get_output(
                    target_policy.output_layer)
                self.target_policy_f_prob_online=tensor_utils.compile_function(
                    inputs=[target_policy.input_var], outputs=target_action)
            else:
    ...
    def train_once(self, itr, paths):
        ...
        for train_itr in range(self.n_train_steps):
            if self.replay_buffer.n_transitions_stored >= self.min_buffer_size:  # noqa: E501
                self.evaluate = True
                train_policy = self.replay_buffer.n_transitions_stored >= self.min_buffer_size * 10
                qf_loss, y, q, policy_loss = self.optimize_policy(epoch, paths, train_policy)
        ...
        if itr % self.n_epoch_cycles == 0:
            ...
            logger.log("Saved")
            if self.evaluate:
                logger.record_tabular('Epoch', int(epoch))
                logger.record_tabular('AllReturns',
                                      self.episode_rewards)
                logger.record_tabular('NumTrajs', len(self.episode_rewards))
    ...
    @overrides
    def optimize_policy(self, itr, samples_data, train_policy=False):
        """
    ...
    def get_itr_snapshot(self, itr, samples_data):
        return dict(itr=itr, policy=self.policy, env=self.env, qf=self.qf)
